"""
WhatsApp Delivery Tracker — v4
New: Route Seq, Performance Score, Store Insights, Daily Insights, Efficiency %
"""
import sys, os
...
from engine import (process_messages, build_delivery_summary,
                    ...
                    build_store_insights)
from reporter import generate_excel

# ════════════════════════════════════════════════════════════════════════════════
st.set_page_config(page_title="WhatsApp Delivery Tracker",
...
.sec-hdr { display:flex; align-items:center; gap:10px; margin:18px 0 10px;
...
def _score_badge(score):
    """Return coloured HTML badge for perf score."""
    try:
        s = int(score)
        cls = 'score-hi' if s >= 80 else ('score-mid' if s >= 50 else 'score-lo')
...
def _eff_color(v):
    try:
        f = float(v)
        return "#10b981" if f >= 80 else ("#f59e0b" if f >= 60 else "#ef4444")
    except Exception:
        return "#94a3b8"
...
STATUS_STYLE = {
    ...
    'High Travel': 'background:#ffe0b2;color:#e65100;font-weight:700',  # orange
...
def style_details(df):
    ...
    for col, hi_t, hi_c, lo_t, lo_c in [
        ('Travel Time (mins)', 60, 'background:#f8d7da;color:#721c24;font-weight:700',
                                30, 'background:#ffe0b2;color:#e65100'),
        ('Store Time (mins)',  30, '',
                                30, 'background:#fff3cd;color:#856404'),
    ]:
        ...
        for i, v in enumerate(df[col]):
            try:
                fv = float(v)
                if hi_c and fv > hi_t:
                    s.at[df.index[i], col] = hi_c
                elif fv > lo_t:
                    s.at[df.index[i], col] = lo_c
            except Exception:
                pass
    if 'Perf Score' in df.columns:
        for i, v in enumerate(df['Perf Score']):
            try:
                sc = int(v)
                bg = ('background:#d4edda;color:#155724' if sc >= 80 else
                      'background:#fff3cd;color:#856404' if sc >= 50 else
                      'background:#f8d7da;color:#721c24')
                s.at[df.index[i], 'Perf Score'] = bg + ';font-weight:700'
            except Exception:
                pass
        ...
        try:
            if float(row.get('Avg Travel Time (mins)') or 0) > 45:
                if 'Avg Travel Time (mins)' in df.columns:
                    s.at[i, 'Avg Travel Time (mins)'] = 'background:#ffe0b2;color:#e65100'
        except Exception:
            pass
        if 'Efficiency %' in df.columns:
            try:
                v = float(row.get('Efficiency %') or 100)
                bg = ('background:#f8d7da;color:#721c24' if v < 50 else
                      'background:#fff3cd;color:#856404' if v < 70 else
                      'background:#d4edda;color:#155724')
                s.at[i, 'Efficiency %'] = bg + ';font-weight:700'
            except Exception:
                pass
    return df.style.apply(lambda _: s, axis=None)
...
def style_summary(df):
    ...
    for i, row in df.iterrows():
        ...
        if 'Efficiency %' in df.columns:
            try:
                ev = float(row.get('Efficiency %') or 100)
                bg = ('background:#f8d7da;color:#721c24' if ev < 50 else
                      'background:#fff3cd;color:#856404' if ev < 70 else
                      'background:#d4edda;color:#155724')
                s.at[i, 'Efficiency %'] = bg + ';font-weight:700'
            except Exception:
                pass
        if 'Avg Perf Score' in df.columns:
            try:
                sc = float(row.get('Avg Perf Score') or 100)
                bg2 = ('background:#d4edda;color:#155724' if sc >= 80 else
                       'background:#fff3cd;color:#856404' if sc >= 50 else
                       'background:#f8d7da;color:#721c24')
                s.at[i, 'Avg Perf Score'] = bg2 + ';font-weight:700'
            except Exception:
                pass
    return df.style.apply(lambda _: s, axis=None)

def style_exc(df):
    warm = {'Long Delay', 'No Activity Gap', 'Break End Without Start'}
    orange = {'High Travel Time'}
    def row_fn(row):
        v = row.get('Exception Type', '')
        if v in orange:
            st_s = 'background:#ffe0b2;color:#e65100;font-weight:700'
        elif v in warm:
            st_s = 'background:#fff3cd;color:#856404;font-weight:700'
        else:
            st_s = 'background:#f8d7da;color:#721c24;font-weight:700'
        return [st_s] * len(row)
    return df.style.apply(row_fn, axis=1)

def style_store_insights(df):
    s = pd.DataFrame('', index=df.index, columns=df.columns)
    for i, row in df.iterrows():
        if 'Missing POD Count' in df.columns:
            try:
                if int(row.get('Missing POD Count') or 0) > 0:
                    s.at[i, 'Missing POD Count'] = 'background:#f8d7da;color:#721c24;font-weight:700'
            except Exception:
                pass
        if 'Delayed Count' in df.columns:
            try:
                if int(row.get('Delayed Count') or 0) > 0:
                    s.at[i, 'Delayed Count'] = 'background:#fff3cd;color:#856404;font-weight:700'
            except Exception:
                pass
    return df.style.apply(lambda _: s, axis=None)
...
with st.spinner("⚙️ Parsing & analysing…"):
    ...
    store_insights = build_store_insights(states)
...
avg_score     = (round(sum(d.get('Perf Score', 100) for d in details) / len(details), 1)
                 if details else 0)
...
with k4: st.markdown(kpi("🟡", total_delayed,  "Delayed",           "#f59e0b"), unsafe_allow_html=True)
with k5: st.markdown(kpi("🔴", total_missing,  "Missing POD",       "#ef4444"), unsafe_allow_html=True)
with k6: st.markdown(kpi("⭐", avg_score,       "Avg Perf Score",
                           "#10b981" if avg_score >= 80 else
                           "#f59e0b" if avg_score >= 50 else "#ef4444"),         unsafe_allow_html=True)
...
df_si       = pd.DataFrame(store_insights) if store_insights else pd.DataFrame()
...
with st.sidebar:
    ...
    if all_dates:
        ...
        sel_dates = [d for d in all_dates
    ...
    if not df_details.empty:
        ...
        st.metric("Avg Perf Score",   avg_score)
    st.markdown("---")
...
srch_f = _f(df_search)
# Store insights not filtered by person/date (global view)
...
with t1:
    if det_f.empty:
        ...
        with r1c3:
            st.markdown("##### ⭐ Avg Perf Score per Person")
            tmp_ps = det_f.copy()
            tmp_ps['_ps'] = pd.to_numeric(tmp_ps.get('Perf Score', pd.Series(dtype=float)), errors='coerce')
            avg_ps = tmp_ps.groupby('Delivery Boy')['_ps'].mean().dropna().round(1)
            if not avg_ps.empty: st.bar_chart(avg_ps.rename("Score"))
        ...
        with r2c3:
            st.markdown("##### 📈 Efficiency % per Person")
            if not sum_f.empty and 'Efficiency %' in sum_f.columns:
                eff_chart = sum_f.groupby('Name')['Efficiency %'].mean().dropna().round(1)
                if not eff_chart.empty:
                    st.bar_chart(eff_chart.rename("Efficiency %"))
                else:
...
with t2:
    ...
    if sum_f.empty:
        ...
        st.caption("🟢 Efficiency ≥70%  |  🟡 50–70%  |  🔴 <50%  |  ⭐ Avg Perf Score (green ≥80, amber 50–79, red <50)")

        st.markdown("#### 🧑 Daily Insight Cards")
        for _, row in sum_f.iterrows():
            eff_v   = _val(row, 'Efficiency %')
            score_v = _val(row, 'Avg Perf Score')
            hrs_v   = _val(row, 'Working Hours')
            eff_col = _eff_color(eff_v)

            with st.expander(
                f"👤 {_val(row, 'Name')}  —  {_val(row, 'Date')}  "
                f"| Eff: {eff_v}%  | Score: {score_v}  | {hrs_v} hrs"
            ):
                # Row 1: delivery counts
                a, b, c, d = st.columns(4)
                a.metric("Total Deliveries", _val(row, 'Total Deliveries (POD)'))
                b.metric("On Time",          _val(row, 'On Time'))
                c.metric("Delayed",          _val(row, 'Delayed'))
                d.metric("Missing POD",      _val(row, 'Missing POD'))

                # Row 2: route & time
                e, f_, g, h = st.columns(4)
                e.metric("Total Routes",     _val(row, 'Total Routes'))
                f_.metric("Working Hours",   f"{hrs_v} hrs" if hrs_v != '—' else '—')
                g.metric("Break Time",
                         f"{_val(row, 'Total Break Time (mins)')} min"
                         if _val(row, 'Total Break Time (mins)') != '—' else '—')
                h.metric("Stores Covered",   _val(row, 'Total Stores Covered'))

                # Row 3: performance & times
                i_, j_, k_, l_ = st.columns(4)
                i_.metric("Avg Perf Score",  score_v)
                j_.metric("Efficiency %",    f"{eff_v}%" if eff_v != '—' else '—')
                k_.metric("First Activity",  _val(row, 'First Activity'))
                l_.metric("Last Activity",   _val(row, 'Last Activity'))

                if 'Route Start (First)' in sum_f.columns:
                    m_, n_ = st.columns(2)
                    m_.metric("Route Start (Departure)", _val(row, 'Route Start (First)'))
                    n_.metric("Route End (Return)",      _val(row, 'Route End (Last)'))
...
with t3:
    ...
    if det_f.empty:
        ...
        st.caption(
            "🟢 OK  |  🟡 Delayed (store >30 min)  |  "
            "🟠 High Travel (>30 min)  |  🔴 Missing POD / Travel >60 min  |  "
            "⭐ Perf Score: green ≥80, amber 50–79, red <50"
        )
        st.markdown("")
...
with t4:
    ...
    if rte_f.empty:
        ...
        st.caption(
            "🟢 Complete / Efficiency ≥70%  |  🟡 50–70%  |  🔴 <50% / Not Ended  |  "
            "🟠 Avg Travel >45 min  |  Route Seq = order within the day"
        )

        st.markdown("#### 🚚 Route Cards")
        for _, row in rte_f.iterrows():
            status   = _val(row, 'Status', 'Unknown')
            seq      = _val(row, 'Route Seq (Day)')
            eff_v    = _val(row, 'Efficiency %')
            sph_v    = _val(row, 'Stores per Hour')
            icon     = "✅" if status == 'Complete' else "❌"

            with st.expander(
                f"{icon}  {_val(row, 'Delivery Boy')}  —  "
                f"Route {_val(row, 'Route No.')} (#{seq} of day)  "
                f"({_val(row, 'Date')})  |  Eff: {eff_v}%"
            ):
                a, b, c, d = st.columns(4)
                ...
                avg_ps  = _val(row, 'Avg Perf Score')
                e.metric("Stores Covered",  stores)
                f_.metric("Avg Store Time", f"{avg_s} min" if avg_s  != '—' else '—')
                g.metric("Avg Travel",      f"{avg_t} min" if avg_t  != '—' else '—')
                h.metric("Avg Perf Score",  avg_ps)

                p, q = st.columns(2)
                p.metric("Stores per Hour",  sph_v)
                q.metric("Efficiency %",     f"{eff_v}%" if eff_v != '—' else '—')

# ─────────────────────────────────────────────────────────────────────────────
# TAB 5 — STORE INSIGHTS
# ─────────────────────────────────────────────────────────────────────────────
with t5:
    sec("🏪", "Store Insights", len(df_si))
    if df_si.empty:
        st.info("No store data available.")
    else:
        st.markdown("Aggregated view of all stores — total visits, avg time, exception counts.")
        st.markdown("")

        # Top-level KPIs
        si1, si2, si3, si4 = st.columns(4)
        si1.metric("Total Unique Stores",    df_si['Store Name'].nunique())
        si2.metric("Total Visits",           int(df_si['Total Visits'].sum()))
        si3.metric("Stores with Missing POD",
                   int((df_si['Missing POD Count'] > 0).sum()))
        si4.metric("Stores with Delays",
                   int((df_si['Delayed Count'] > 0).sum()))

        st.markdown("")
        st.dataframe(style_store_insights(df_si), use_container_width=True,
                     height=min(600, 60 + len(df_si) * 38))
        st.caption(
            "🔵 ≥5 visits  |  🔴 Missing POD count > 0  |  🟡 Delayed count > 0  |  "
            "Sorted by most visited"
        )

        # Top 10 most-visited bar chart
        st.markdown("")
        st.markdown("##### 🏆 Top 10 Most Visited Stores")
        top10 = df_si.nlargest(10, 'Total Visits').set_index('Store Name')['Total Visits']
        st.bar_chart(top10)

        # Avg time per store
        if 'Avg Store Time (mins)' in df_si.columns:
            st.markdown("##### ⏱ Avg Store Time — Top 10 Stores by Visits")
            top10_time = (df_si.nlargest(10, 'Total Visits')
                          .set_index('Store Name')['Avg Store Time (mins)']
                          .apply(pd.to_numeric, errors='coerce').dropna())
            if not top10_time.empty:
                st.bar_chart(top10_time)
...
with fc1:
    st.caption(
        "WhatsApp Delivery Tracker v4 · "
        "Route Seq · Perf Score · Store Insights · Efficiency % · "
        "Android & iOS formats"
    )
with fc2:
    st.download_button(
